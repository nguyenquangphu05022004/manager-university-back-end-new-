...
public class UserServiceImpl implements UserService{
    ...
    @Override
    public boolean isPasswordMatch(String rawPass, String encode) {
    ...
    public void resetPassword(Long userId, MemberResetPasswordVO memberResetPasswordVO) {
        User user = this.getById(userId);
        if(isPasswordMatch(memberResetPasswordVO.getOldPass(), user.getPassword())) {
    ...
    public User getByUsername(String username) {
        return this.userRepository.findUserByUsernameIgnoreCase(username).orElse(null);
    }
}
